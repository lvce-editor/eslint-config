...
void test('disallows element helpers in the recommended preset', () => {
  const code = `element(VirtualDomElements.Div, { className: ClassNames.DrawView }, [])`
...
void test('restricts dependency versions in root and workspace manifests', () => {
  ...
  for (const config of [recommended, strict]) {
    for (const filename of ['package.json', 'packages/worker/package.json']) {
      const messages = linter.verify('{"devDependencies":{"typescript":"7.0.2","eslint":"11.0.0"}}', config, { filename })
      deepEqual(
